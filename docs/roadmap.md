...
7. Continue hardening the isolated Bash runner. The first opt-in boundary now runs in a separate read-only, unprivileged, resource-limited container on an internal control network, with clean process environments, bounded logs, timeouts, and durable execution results. Next steps are active cancellation, per-run ephemeral isolation, and stronger platform-specific sandboxing.
