...
6. Extend the workspace module catalog with version upgrades. Exact-version installation, enable/disable controls, publisher trust, HTTP host permissions, secret-access review, and referenced-module protection are now present. A later registry release can surface newer versions in the same catalog without changing existing stages, which remain pinned to their configured version.
