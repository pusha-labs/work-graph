...
type ActivityItem = { id: string; eventType: string; summary: string; detail: string; actorName: string; occurredAt: string; workspaceRevision: number | null; nodeId: string | null; canPreview: boolean };
...
type Diagnostic = { id:string; kind:'missing_requester'|'no_eligible_performer'|'open_descendants'|'uncovered_knowledge'|'concentrated_knowledge'; severity:'warning'|'error'; title:string; explanation:string; nodeId:string; nodeTitle:string; subjectId:string; subjectName:string; evidence:string[]; relatedNodeIds:string[] };
          ...
          {!currentWorkspace ? (
            ...
            <>
              ...
              <div className="detail-grid">
                ...
                <ActivityPanel items={activity} activeRevision={previewRevision} busy={busy} historyMode={view==='history'} onPreview={(revision,entityId)=>{setView('history');void previewAtRevision(revision,entityId)}} />
              </div>
            </>
          )}
        </section>
...
function MyWorkView({items,rootScores,criticalNodeIds,onOpen}:{items:MyWorkItem[];rootScores:Record<string,number>;criticalNodeIds:Set<string>;onOpen:(nodeId:string)=>void}){
  ...
  return <section className="my-work-view"><header><div><p className="eyebrow">Personal queue</p><h2>My work</h2></div><p>Active and available steps from every branch, ordered by current context. Open an item in the tree to act with its purpose and surrounding work visible.</p></header>{ordered.length===0?<div className="my-work-empty"><h3>Nothing is waiting for you</h3><p>Work appears here when an active step matches all of your roles, skills, and entity knowledge.</p></div>:<div className="my-work-list">{ordered.map((item)=>{const critical=Boolean(item.criticalUntil||criticalNodeIds.has(item.nodeId));return <article key={item.stepId} className={`${item.stepStatus} ${critical?'critical':''}`}><div className="my-work-state"><span>{item.stepStatus}</span>{critical&&<i>Critical</i>}</div><div className="my-work-copy"><small>{item.rootTitle}</small><h3>{item.stepName}</h3><p>{item.nodeTitle}</p><em>Requested by {item.requester} · step {item.stepPosition}</em></div><div className="my-work-actions"><button onClick={()=>onOpen(item.nodeId)}>Show in tree</button></div></article>})}</div>}</section>;
}
...
function ExchangeView({offers,busy,perform,onSubmit,onWithdraw,onOpen}:{offers:ExchangeOffer[];busy:boolean;perform:(action:()=>Promise<void>)=>Promise<void>;onSubmit:(offer:ExchangeOffer,minutes:number)=>Promise<void>;onWithdraw:(offer:ExchangeOffer)=>Promise<void>;onOpen:(nodeId:string)=>void}){
  return <section className="exchange-view"><header><div><p className="eyebrow">Human-step exchange</p><h2>Available work</h2></div><p>These ready steps match your skills and entity knowledge. Propose the time you expect your own step to take.</p></header>{offers.length===0?<div className="my-work-empty"><h3>No matching offers right now</h3><p>New work appears here when a ready human step matches every required capability and knowledge subject.</p></div>:<div className="exchange-list">{offers.map((offer)=><ExchangeOfferCard key={`${offer.stepId}-${offer.myBid?.updatedAt??'new'}`} offer={offer} busy={busy} perform={perform} onSubmit={onSubmit} onWithdraw={onWithdraw} onOpen={onOpen}/>)}</div>}</section>;
}
...
function NodeEditor({ node, circle, capabilities, knowledgeSubjects, workflowModules, criticality, busy, onAddRequirement, onAddKnowledgeRequirement, onAddWorkflowStep, onUpdateWorkflowStep, onMoveWorkflowStep, onDeleteWorkflowStep,onSelectBid, onReturn, onMarkCritical, onRevokeCritical, onAction, onSave }: { node: WorkNode; circle: WorkCircle | null; capabilities: Capability[]; knowledgeSubjects: KnowledgeSubject[]; workflowModules: WorkflowModule[]; criticality: CriticalitySignal[]; busy: boolean; onAddRequirement: (capabilityId: string) => void; onAddKnowledgeRequirement: (subjectId: string) => void; onAddWorkflowStep: (input: NewWorkflowStep) => void; onUpdateWorkflowStep: (stepId:string,input:WorkflowStepUpdate) => void; onMoveWorkflowStep: (stepId:string,direction:'earlier'|'later') => void; onDeleteWorkflowStep: (stepId:string) => void;onSelectBid:(stepId:string)=>void; onReturn: (stepId:string,reason:string)=>void; onMarkCritical: (reason: string, until: string) => void; onRevokeCritical: (signalId: string) => void; onAction: (action: WorkAction) => void; onSave: (input: Pick<WorkNode, 'title' | 'desiredOutcome' | 'lifecycleStatus'>) => void }) {
  ...
  return <article className="editor">
    <div className="editor-top"><div><p className="eyebrow">{node.parentId ? 'Task' : 'Root goal'} · revision {node.updatedRevision}</p><h2>{node.title}</h2></div></div>
    <div className="task-summary"><span className={`task-status ${status}`}><i className={`dot ${status}`}/>{status}</span><p>{node.desiredOutcome||'No desired outcome has been described yet.'}</p></div>
    <details className="task-details"><summary>Edit task details</summary><form onSubmit={(event) => { event.preventDefault(); onSave({ title, desiredOutcome: outcome, lifecycleStatus: status }); }}>
      ...
      <label>Status<select value={status} disabled><option value="planned">Planned</option><option value="active">Active</option><option value="blocked">Blocked</option><option value="review">Awaiting requester review</option><option value="closed">Closed</option></select></label>
      <div className="form-actions"><span>Changes are preserved in the workspace history.</span><button disabled={busy}>{busy ? 'Saving…' : 'Save changes'}</button></div>
    </form></details>
...
function WorkCirclePanel({ canEdit, circle, capabilities, knowledgeSubjects, workflowModules, busy, onAddRequirement, onAddKnowledgeRequirement, onAddWorkflowStep, onUpdateWorkflowStep, onMoveWorkflowStep, onDeleteWorkflowStep,onSelectBid, onReturn, onAction }: { canEdit: boolean; circle: WorkCircle | null; capabilities: Capability[]; knowledgeSubjects: KnowledgeSubject[]; workflowModules: WorkflowModule[]; busy: boolean; onAddRequirement: (capabilityId: string) => void; onAddKnowledgeRequirement: (subjectId: string) => void; onAddWorkflowStep: (input: NewWorkflowStep) => void; onUpdateWorkflowStep: (stepId:string,input:WorkflowStepUpdate) => void; onMoveWorkflowStep: (stepId:string,direction:'earlier'|'later') => void; onDeleteWorkflowStep: (stepId:string) => void;onSelectBid:(stepId:string)=>void; onReturn:(stepId:string,reason:string)=>void; onAction: (action: WorkAction) => void }) {
  ...
  return <section className="work-circle"><div className="circle-heading"><div><p className="eyebrow">Task route</p><h3>Task circle</h3><p>Work moves from the requester through each step, then returns to the requester for acceptance.</p></div><span>{circle.workflowSteps.length} step{circle.workflowSteps.length===1?'':'s'}</span></div>
    <div className="circle-route" aria-label="Task execution order">
      <div className="route-segment"><article className="route-card requester"><span className="route-number">●</span><div><small>Starts here · requester</small><strong>{circle.requester.displayName}</strong><em>Defines the desired result</em></div></article><span className="route-arrow">→</span></div>
      {circle.workflowSteps.map((step)=><div className="route-segment" key={step.id}><article className={'route-card '+step.stepStatus+' '+step.stepType+' editable'} role="button" tabIndex={0} onClick={()=>setEditingStepId(step.id)}><span className="route-number">{step.position}</span><div><small>{step.stepStatus==='active'?'Happening now':step.stepStatus==='assigned'?'Performer selected':step.stepStatus==='ready'?'Ready to start':step.stepStatus==='completed'?'Completed':step.stepStatus==='failed'?'Failed or cancelled':'Waiting'} · {step.stepType==='api'?'HTTP':step.stepType==='script'?'Bash':step.stepType}</small><strong>{step.name}</strong><em>{step.stepType==='human'?(step.claimedBy?.displayName??([...step.requirements.map((item)=>item.name),...step.knowledgeRequirements.map((item)=>item.name)].join(' + ')||'Add matching requirements')):step.stepType==='api'?String(step.configuration.url??'HTTP module draft'):'Isolated runner required'}</em></div></article><span className="route-arrow">→</span></div>)}
      {canEdit&&<div className="route-segment add-segment">{addingStep?<form className="add-step-popover step-composer" onSubmit={addStep}><div className="composer-heading"><strong>Add next step</strong><small>{stepType?'Configure the selected step':'Describe who or what should act'}</small></div>{!stepType&&<><input autoFocus aria-label="Next step intent" placeholder="Try: Python, Product A, bash, API request…" value={stepIntent} onChange={(event)=>setStepIntent(event.target.value)} /><div className="composer-results">{moduleMatches.map((module)=><button type="button" key={module.moduleId} onClick={()=>chooseModule(module)}><b>{module.stepType==='api'?'↗':module.stepType==='script'?'⌘':'◇'}</b><span><strong>{module.name}</strong><small>{module.description}</small></span></button>)}{humanMatches.map((item)=><button type="button" key={`${item.source}-${item.id}`} onClick={()=>{setStepType('human');setStepName(stepIntent.trim()||item.label);if(item.source==='capability')setStepCapability(item.id);else setStepSubject(item.id)}}><b>●</b><span><strong>{item.label}</strong><small>{item.kind} · human work</small></span></button>)}{normalizedIntent&&!humanMatches.length&&!moduleMatches.length&&<p>No exact match. Create the role, skill, or entity in Directory first.</p>}</div></>}{stepType&&<><div className={`selected-step-kind ${stepType}`}><b>{stepType==='human'?'●':stepType==='api'?'↗':'⌘'}</b><span><strong>{stepType==='human'?'Human work':selectedModule?.name??'Module'}</strong><small>{stepType==='human'?'Matched from your directory':((selectedModule?.publisher??'Unknown publisher')+' · '+(selectedModule?.moduleVersion??''))}</small></span><button type="button" onClick={()=>setStepType(null)}>Change</button></div><input aria-label="Step name" placeholder="Step name" value={stepName} required onChange={(event)=>setStepName(event.target.value)} />{stepType==='human'&&<><select aria-label="Step role or skill" value={stepCapability} onChange={(event)=>setStepCapability(event.target.value)}><option value="">Role or skill (optional)…</option>{capabilities.map((item)=><option key={item.id} value={item.id}>{item.name} · {item.capabilityType}</option>)}</select><select aria-label="Step entity knowledge" value={stepSubject} onChange={(event)=>setStepSubject(event.target.value)}><option value="">Entity knowledge (optional)…</option>{knowledgeSubjects.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></>}{selectedModule&&<ModuleConfigurationEditor module={selectedModule} value={moduleConfiguration} onChange={setModuleConfiguration} />}<p className="runner-notice">Saved as a configured step. Execution stays disabled until the isolated runner is installed.</p></>}<div className="composer-actions"><button type="button" className="secondary" onClick={resetComposer}>Cancel</button>{stepType&&<button disabled={busy||!stepName.trim()||missingRequiredModuleField}>Add to circle</button>}</div></form>:<button className="add-step-button" onClick={()=>setAddingStep(true)}><span>+</span><strong>Add next step</strong><small>Person, API, script or module</small></button>}<span className="route-arrow">→</span></div>}
      <article className="route-card requester final"><span className="route-number">✓</span><div><small>Returns here · final acceptance</small><strong>{circle.requester.displayName}</strong><em>Accepts or returns a step</em></div></article>
    </div>
    {editingStepId&&<WorkflowStepEditor editable={canEdit} canSelect={circle.requester.id===circle.currentActor.id} key={editingStepId} step={circle.workflowSteps.find((step)=>step.id===editingStepId)!} stepCount={circle.workflowSteps.length} capabilities={capabilities} knowledgeSubjects={knowledgeSubjects} modules={workflowModules} busy={busy} onClose={()=>setEditingStepId('')} onSave={(input)=>{onUpdateWorkflowStep(editingStepId,input);setEditingStepId('')}} onMove={(direction)=>onMoveWorkflowStep(editingStepId,direction)} onDelete={()=>{onDeleteWorkflowStep(editingStepId);setEditingStepId('')}} onSelectBid={()=>{onSelectBid(editingStepId);setEditingStepId('')}} />}
    <div className="workflow-actions">{circle.permissions.canClaim && <button disabled={busy} onClick={() => onAction('claim')}>Start current step</button>}{circle.permissions.canSubmit && <button disabled={busy} onClick={() => onAction('submit')}>Complete current step</button>}{circle.permissions.canClose && <button disabled={busy} onClick={() => onAction('close')}>Accept and close</button>}{circle.permissions.canReopen && <button className="secondary" disabled={busy} onClick={() => onAction('reopen')}>Reopen task</button>}{circle.permissions.canCancel && <button className="danger" disabled={busy} onClick={() => onAction('cancel')}>Cancel HTTP execution</button>}{circle.permissions.canRetry && <button disabled={busy} onClick={() => onAction('retry')}>Retry failed HTTP step</button>}</div>
    <details className="matching-details"><summary><span>Current step matching</span><small>{circle.matchingPerformers.length} matching performer{circle.matchingPerformers.length===1?'':'s'} · edit requirements</small></summary><div className="requirements-block"><h4>Required roles and skills</h4><div className="capability-chips">{circle.requirements.map((requirement) => <span key={requirement.id} className={requirement.capabilityType}>{requirement.name}</span>)}{circle.requirements.length === 0 && <small>No role or skill requirements yet</small>}</div>{available.length > 0 && <div className="assign-capability"><select aria-label="Task requirement" value={selected} onChange={(event) => setSelected(event.target.value)}><option value="">Add role or skill…</option>{available.map((capability) => <option key={capability.id} value={capability.id}>{capability.name} · {capability.capabilityType}</option>)}</select><button disabled={busy || !selected} onClick={() => { onAddRequirement(selected); setSelected(''); }}>Add</button></div>}<h4 className="knowledge-requirement-title">Required entity knowledge</h4><div className="knowledge-requirement-chips">{circle.knowledgeRequirements.map((requirement) => <span key={requirement.id}>{requirement.name}<small>{requirement.subjectType}</small></span>)}{circle.knowledgeRequirements.length === 0 && <small>No entity knowledge required yet</small>}</div>{availableSubjects.length > 0 && <div className="assign-capability"><select aria-label="Task knowledge requirement" value={selectedSubject} onChange={(event) => setSelectedSubject(event.target.value)}><option value="">Add entity knowledge…</option>{availableSubjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name} · {subject.subjectType}</option>)}</select><button disabled={busy || !selectedSubject} onClick={() => { onAddKnowledgeRequirement(selectedSubject); setSelectedSubject(''); }}>Add</button></div>}<div className="matches-block"><h4>Matching performers · strongest evidence first</h4>{circle.requirements.length === 0 && circle.knowledgeRequirements.length === 0 ? <p>Add requirements to calculate candidates.</p> : circle.matchingPerformers.length ? <div>{circle.matchingPerformers.map((actor,index) => <span key={actor.id} className={actor.matchQuality}><b>{index+1}</b>{actor.displayName}<small>{actor.matchQuality==='verified'?'Admin verified':actor.matchQuality==='partially_verified'?`${actor.confirmedRequirements}/${actor.requirementCount} verified`:'Self-reported'}</small></span>)}</div> : <p>No actor currently satisfies every requirement.</p>}</div></div></details>
    ...
    {circle.permissions.canReturn&&<form className="return-work" onSubmit={(event)=>{event.preventDefault();onReturn(returnStep,returnReason);setReturnStep('');setReturnReason('');}}><div><h4>Return for revision</h4><p>Choose the step that must be repeated. Every later step will wait again.</p></div><select aria-label="Step to repeat" value={returnStep} required onChange={(event)=>setReturnStep(event.target.value)}><option value="">Choose completed step…</option>{circle.workflowSteps.filter((step)=>step.stepStatus==='completed').map((step)=><option key={step.id} value={step.id}>{step.position}. {step.name}</option>)}</select><textarea aria-label="Revision reason" placeholder="Explain what needs to change" value={returnReason} required rows={3} onChange={(event)=>setReturnReason(event.target.value)} /><button className="secondary" disabled={busy}>Return selected step</button></form>}
  </section>;
}

function WorkflowStepEditor({editable,canSelect,step,stepCount,capabilities,knowledgeSubjects,modules,busy,onClose,onSave,onMove,onDelete,onSelectBid}:{editable:boolean;canSelect:boolean;step:WorkflowStep;stepCount:number;capabilities:Capability[];knowledgeSubjects:KnowledgeSubject[];modules:WorkflowModule[];busy:boolean;onClose:()=>void;onSave:(input:WorkflowStepUpdate)=>void;onMove:(direction:'earlier'|'later')=>void;onDelete:()=>void;onSelectBid:()=>void}) {
      ...
      {items.map((item) => {
        return <li key={item.id}>
          <button className={activeRevision === item.workspaceRevision ? 'active' : ''} disabled={busy || !item.canPreview} onClick={() => item.canPreview && item.workspaceRevision && item.nodeId && onPreview(item.workspaceRevision, item.nodeId)}>
            ...
            <span className="event-copy"><strong>{item.summary}</strong><small>{item.detail}</small><time>{item.actorName} · {new Date(item.occurredAt).toLocaleString()}</time></span>
          </button>
        </li>;
      })}
    </ol>}</div>
...
function DirectoryView({ workspaceId, workspace, directory, knowledgeSubjects, busy, perform, onDistributionChange, onCreateActor, onCreateCapability, onAssignCapability,onRemoveCapability,onUpdateMemberRole,onRemoveMember, onCreateKnowledgeSubject, onAssignKnowledge,onRemoveKnowledge }: {
  ...
  return <section className="directory-view">
    ...
    {canAdminister&&<section className="distribution-policy"><div><strong>Work distribution</strong><span>{workspace.workDistributionMode==='simple'?'People can start matching work directly. Best for personal projects and small teams.':'Matching people propose a duration before human steps are assigned.'}</span></div><select aria-label="Workspace work distribution" value={workspace.workDistributionMode} disabled={busy} onChange={(event)=>perform(()=>onDistributionChange(event.target.value as Workspace['workDistributionMode']))}><option value="simple">Simple · direct start</option><option value="exchange">Exchange · duration estimates</option></select></section>}
