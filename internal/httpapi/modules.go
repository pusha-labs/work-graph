...
func (s *server) listModuleInstallations(w http.ResponseWriter, r *http.Request) {
	rows, err := s.db.Query(r.Context(), `SELECT m.module_id,m.module_version,m.name,m.description,m.step_type,m.search_terms,m.configuration_schema,m.publisher,i.enabled,i.allowed_hosts,i.allow_secrets,i.publisher_trusted FROM workflow_modules m JOIN workspace_module_installations i ON i.module_id=m.module_id AND i.module_version=m.module_version WHERE i.workspace_id=$1 ORDER BY m.name,m.module_version DESC`, r.PathValue("workspaceID"))
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ModuleID, &item.ModuleVersion, &item.Name, &item.Description, &item.StepType, &item.SearchTerms, &item.ConfigurationSchema, &item.Publisher, &item.Enabled, &item.AllowedHosts, &item.AllowSecrets, &item.PublisherTrusted); err != nil {
