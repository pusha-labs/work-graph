...
The Bash runner is a separate opt-in service and the Bash module is disabled in every workspace by default. An administrator must explicitly enable its exact trusted version before any script can be created or leased. Start the runner with `RUNNER_TOKEN=... docker compose --profile bash-automation up -d bash-runner`. The container is read-only, unprivileged, has all Linux capabilities dropped, cannot access the Docker socket, runs with CPU, memory, process, output, and time limits, and is attached only to an internal control network shared with the API. Scripts receive a clean environment without the runner token or workspace secrets. Requester cancellation terminates the complete process group, and failed or cancelled automation can be retried while preserving every earlier attempt. This is the first isolation boundary; do not weaken these controls in production.
