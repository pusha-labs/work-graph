  ...
  return <section className="work-circle"><div className="circle-heading"><div><p className="eyebrow">Task route</p><h3>Task circle</h3><p>Work moves from the requester through each step, then returns to the requester for acceptance.</p></div><span>{circle.workflowSteps.length} step{circle.workflowSteps.length===1?'':'s'}</span></div>
    <div className="circle-loop" aria-label="Task execution circle">
      ...
      <div className={`circle-map items-${Math.min(circleItemCount,9)}`} aria-label="Task execution order">
        ...
        {circle.workflowSteps.map((step,index)=><button type="button" key={step.id} className={`circle-node step ${step.stepStatus} ${step.stepType} ${currentStep?.id===step.id&&!requesterHasBall?'has-ball':''}`} style={{left:`${circlePoints[index+1].x}%`,top:`${circlePoints[index+1].y}%`}} onClick={()=>setEditingStepId(step.id)}><span className="circle-node-number">{step.position}</span><small>{step.stepType==='api'?'HTTP':step.stepType==='script'?'Bash':step.stepType}</small><strong>{step.name}</strong><em>{step.stepStatus==='active'?'Happening now':step.stepStatus==='assigned'?'Performer selected':step.stepStatus==='ready'?'Ready to start':step.stepStatus==='completed'?'Completed':step.stepStatus==='failed'?'Needs attention':'Waiting'}</em>{currentStep?.id===step.id&&!requesterHasBall&&<span className="card-ball" title="Current responsibility" />}</button>)}
        {canEdit&&<button type="button" className="circle-node add" style={{left:`${circlePoints[circleItemCount-1].x}%`,top:`${circlePoints[circleItemCount-1].y}%`}} onClick={()=>setAddingStep(true)}><span className="circle-node-number">+</span><strong>Add step</strong><em>Person or automation</em></button>}
