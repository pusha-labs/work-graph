...
type WorkflowStepUpdate = { name: string; capabilityId?: string; subjectId?: string; distributionMode?:DistributionMode; configuration?: Record<string, unknown>; moduleVersion?:string };
...
type ModuleInstallation=WorkflowModule&{installed:boolean;enabled:boolean;allowedHosts:string[];allowSecrets:boolean;publisherTrusted:boolean};
...
function App() {
  ...
  async function installModule(item:ModuleInstallation){await api(`/api/v1/workspaces/${workspaceId}/module-installations/${encodeURIComponent(item.moduleId)}/${encodeURIComponent(item.moduleVersion)}`,{method:'POST'});setModuleInstallations(await api<ModuleInstallation[]>(`/api/v1/workspaces/${workspaceId}/module-installations`));}
      ...
      <section className="workspace" id="work">
        ...
        <section className="canvas">
          {!currentWorkspace ? (
            ...
            <AdminSurface title="Modules"><ModulesView items={moduleInstallations} busy={busy} perform={perform} onInstall={installModule} onSave={updateModulePolicy}/></AdminSurface>
          ) : view === 'api-access' ? (
...
function WorkflowStepEditor({editable,canSelect,step,stepCount,capabilities,knowledgeSubjects,modules,busy,onSave,onMove,onDelete,onSelectBid}:{editable:boolean;canSelect:boolean;step:WorkflowStep;stepCount:number;capabilities:Capability[];knowledgeSubjects:KnowledgeSubject[];modules:WorkflowModule[];busy:boolean;onSave:(input:WorkflowStepUpdate)=>void;onMove:(direction:'earlier'|'later')=>void;onDelete:()=>void;onSelectBid:()=>void}) {
  ...
  const [moduleVersion,setModuleVersion]=useState(step.moduleVersion??'');
  const compatibleModules=modules.filter((item)=>item.moduleId===step.moduleId);
  const module=compatibleModules.find((item)=>item.moduleVersion===moduleVersion)??null;
  const chooseVersion=(version:string)=>{setModuleVersion(version);const selected=compatibleModules.find((item)=>item.moduleVersion===version);if(selected)setConfiguration((current)=>({...Object.fromEntries(selected.configurationSchema.fields.filter((field)=>field.default!==undefined).map((field)=>[field.key,field.default!])),...current}))};
...
function ModulesView({items,busy,perform,onInstall,onSave}:{items:ModuleInstallation[];busy:boolean;perform:(action:()=>Promise<void>)=>Promise<void>;onInstall:(item:ModuleInstallation)=>Promise<void>;onSave:(item:ModuleInstallation,policy:Pick<ModuleInstallation,'enabled'|'allowedHosts'|'allowSecrets'|'publisherTrusted'>)=>Promise<void>}){
  ...
  return <section className="modules-view"><header><div><p className="eyebrow">Module catalog</p><h2>Workflow modules</h2></div><p>Install an exact version, review its publisher and permissions, then enable it. Existing stages remain pinned to the version they were created with.</p></header><div className="module-policy-list">{items.map((item)=>{const draft=draftFor(item);const installedPeer=items.find((candidate)=>candidate.moduleId===item.moduleId&&candidate.installed);const isUpdate=!item.installed&&!!installedPeer&&compareModuleVersions(item.moduleVersion,installedPeer.moduleVersion)>0;const previousFields=new Set(installedPeer?.configurationSchema.fields.map((field)=>field.key)??[]);const addedFields=item.configurationSchema.fields.filter((field)=>!previousFields.has(field.key)).map((field)=>field.label);return <article key={`${item.moduleId}@${item.moduleVersion}`} className={!item.installed?'available':''}><div className="module-policy-heading"><span className={`module-kind ${item.stepType}`}>{item.stepType}</span><div><strong>{item.name}</strong><small>{item.publisher} · {item.moduleVersion}</small></div>{item.installed?<label className="policy-toggle"><input type="checkbox" checked={draft.enabled} disabled={!draft.publisherTrusted} onChange={(event)=>update(item,{enabled:event.target.checked})}/> Enabled</label>:<span className="module-status">{isUpdate?'Update available':'Available'}</span>}</div><p>{item.description}</p>{!item.installed?<div className="module-install"><span>{isUpdate?`Updates ${installedPeer?.moduleVersion} → ${item.moduleVersion}. ${addedFields.length?`New settings: ${addedFields.join(', ')}.`:'Configuration fields are compatible.'}`:'This exact version is not installed in this workspace.'}</span><button disabled={busy} onClick={()=>perform(()=>onInstall(item))}>{isUpdate?'Install update':'Install version'}</button></div>:<><div className="publisher-trust"><label className="policy-toggle"><input type="checkbox" checked={draft.publisherTrusted} onChange={(event)=>update(item,{publisherTrusted:event.target.checked,enabled:event.target.checked?draft.enabled:false})}/> Trust publisher: {item.publisher}</label><small>Trust applies to this exact module version in this workspace.</small></div>{item.stepType==='api'&&<div className="module-policy-fields"><label>Exact allowed hosts<input value={draft.hosts} placeholder="api.example.com, hooks.example.com" onChange={(event)=>update(item,{hosts:event.target.value})}/><small>No schemes, paths, ports, or wildcards.</small></label><label className="policy-toggle"><input type="checkbox" checked={draft.allowSecrets} onChange={(event)=>update(item,{allowSecrets:event.target.checked})}/> Allow this module to use workspace secrets</label></div>}<button disabled={busy||draft.enabled&&!draft.publisherTrusted} onClick={()=>perform(()=>onSave(item,{enabled:draft.enabled,allowedHosts:draft.hosts.split(',').map((host)=>host.trim()).filter(Boolean),allowSecrets:draft.allowSecrets,publisherTrusted:draft.publisherTrusted}))}>Save policy</button></>}</article>})}</div></section>;
}

function compareModuleVersions(left:string,right:string){const a=left.split('.').map(Number),b=right.split('.').map(Number);for(let index=0;index<Math.max(a.length,b.length);index++){const difference=(a[index]??0)-(b[index]??0);if(difference)return difference}return 0}
