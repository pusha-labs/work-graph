...
6. Continue hardening module upgrades. The catalog now detects newer versions, compares configuration fields, installs each version with independent trust and permissions, and lets users migrate only unstarted stages. Existing and executed stages remain pinned to their recorded version.
7. Continue hardening the isolated Bash runner. The opt-in boundary now runs in a separate read-only, unprivileged, resource-limited container on an internal control network, with clean process environments, bounded logs, timeouts, active process-group cancellation, retry history, and durable execution results. Next steps are per-run ephemeral isolation and stronger platform-specific sandboxing.
